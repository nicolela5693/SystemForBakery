...
public class Order_DBcontrol {
	...
	public Order_DBcontrol() {
		{
			try {
				// Class 的靜態 forName() 方法實現動態加載類別
				Class.forName("com.mysql.jdbc.Driver");
				// 3306|MySQL開放此端口
				con = DriverManager.getConnection("jdbc:mysql://localhost:3306/bread?serverTimezone=UTC  ", "root",
						"FU23310521");
				//FU23310521
	...
	public void sendorderData(String C_id, String customerorderdate, String Aamount,String Bamount, String Camount, String totalprice, String due_date) 
	{
		
	 try {
		String query =
		"INSERT INTO `bread`.`order` (`Customer_id`, `Date`, `A_amount`, `B_amount`, `C_amount`, `Totalmoney`, `Deliver_Date`) VALUES"
		+ "('" + C_id + "','" + customerorderdate + "','" + Aamount + "','" + Bamount + "','" + Camount + "','" + totalprice + "','" + due_date + "')";
	...
	public String findid(String phone)
	{
		String id="";
		try {
			String query = "SELECT C_Id FROM customer WHERE TEL=" + "'"+ phone + "'" ;
			rs4 = st.executeQuery(query);
			while (rs4.next()) {
			 id=rs4.getString("C_Id");
			}
		} catch (Exception ex) {
			System.out.println(ex);
		}
